...
class EchoStateNetwork(nn.Module):
    ...
    def solve_supervised(self, y, return_states, SCALE):
        ...
        #assert False, f'biases {biases[0].shape} biases {biases[1].shape}, weights[0].shape {weights[0].shape} ... {weights[1].shape}'

        self.LinOut.weight = Parameter(hstack(weights).T)#.view(self.n_outputs, -1))
        self.LinOut.bias = Parameter(hstack(biases).view(-1, self.n_outputs))#bias.view(1, self.n_outputs))
